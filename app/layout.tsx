...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const favicon = s.branding.faviconUrl || "/favicon.ico";

  return {
    title: `${siteName} | Ingressos`,
    description: "Compre ingressos para eventos no Lorde Nelson Rest Pub - Maceió. Programação, shows, forró e jogos.",
    icons: { icon: favicon },
  };
}
      ...
      <body className="min-h-full flex flex-col bg-zinc-950 text-zinc-200">
        ...
        <footer className="border-t border-white/10 py-10 text-xs text-zinc-500">
          <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-y-4">
            <div dangerouslySetInnerHTML={{ __html: footerLeft || `Lorde Nelson Rest Pub • Rua Silvério Jorge, 241, Jaraguá — Maceió/AL<br/>Qui a Sáb • 20h às 02h` }} />
            <div className="md:text-right" dangerouslySetInnerHTML={{ __html: footerRight || `© ${new Date().getFullYear()} ${siteName}. Portal moderno de ingressos.` }} />
          </div>
        </footer>
